...
module.exports.run = async (client, message, args) => {
    if(args.length !== 3) {
        await message.channel.send("__**ATTENTION IL N'Y A PAS ASSEZ D'ARGUMENTS POUR AJOUTER CETTE POS !**__")
        return
    }

    if(!parseInt(args[1]) || !parseInt(args[2])) {

        await message.channel.send("__**ATTENTION LES DEUX DERNIERS ARGUMENTS DOIVENT ETRE DES NUMEROS !**__")
        return
    } 
